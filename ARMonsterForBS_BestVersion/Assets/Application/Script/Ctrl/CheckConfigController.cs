...
public class CheckConfigController : BaseController {
    ...
    {
        if(checkConfigData == null) checkConfigData=  new CheckConfigData();
    ...
    {
        AndaDataManager.Instance.CallServerGetConfig(CallBackGetConfigBase);
    }

    private void CallServerGetConfig(List<ConfigFile> configFiles )
    {
        AndaDataManager.Instance.CallServerUpdataConfig(configFiles,CallBackGetConfigFiles);
    }
    ...
    private void TrueEnd()
    {
        ...
        mainContoller.SwitchCtrl("loginCtrl");
